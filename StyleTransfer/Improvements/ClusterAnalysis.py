...
from FeatureMap_K_Mode import Clusterer
...
def prepare_style_loss(style, combination, binary_solution):
    ...
    return sl/binary_solution.count(1)      # Important normalization given by the amount of feature maps that are being
...
for layer_name in Learner.feature_layers:
    ...
    c = Clusterer()
    solutions, clusters = c.get_clusters()


    for solution, cluster in zip(solutions, clusters):

        #starting_set = get_solutions(total_style_reference_features, total_style_combination_features)
        starting_set = get_solution(total_style_reference_features, total_style_combination_features, solution)

        print("Analysing Solution: ", solution)
        print("Clustered in cluster: ", cluster)
        ...
        sl = prepare_style_loss(starting_feature_maps, starting_reference_maps, binary_solution)
        loss += (style_weight / len(Learner.feature_layers)) * sl                    
