...
def make_original_pool(total_style_reference_features, total_style_combination_features):
    ...
    """
    Create a set of solutions of length 512 represented by binary values representing if a feature map is used or not
    """

    genetic_population = list()

    feature_population = list()
    reference_population = list()

    dims = total_style_reference_features.get_shape()
    d = dims[-1]

    for n_solutions in range(0, 10):
        ...
        for feature_layer in range(0, 512):
            tmp_bit = random.randint(0,1)
            solution.append(tmp_bit)
        ...
        feature_population.append(tf.stack(single_feat_solution, axis=2))
        reference_population.append(tf.stack(single_ref_solution, axis=2))

        genetic_population.append(solution)

    return(feature_population, reference_population, genetic_population)

def filter_and_make_new_generation(total_style_reference_features, total_content_reference_features, solutions):

    def remove_duplicate_soultions(dictionary):     #Remove duplicate solutions
            raw_population = {}
    ...
    filtered_solutions = remove_duplicate_soultions(solutions)

    ground_truth_loss = max(filtered_solutions.values())
    survival_threshold = (survival_chance*ground_truth_loss)/100
    thresholded_solutions = {k: v for k, v in filtered_solutions.iteritems() if v >= survival_threshold}

    tmp_parents_for_breeding = [tensor for tensor in thresholded_solutions.keys()]
    parents_for_breeding = list()

    for parent in tmp_parents_for_breeding: #Treat solutions as lists and not as strings anymore
        chromosome = ''.join(c for c in parent if c.isdigit())
        parents_for_breeding.append(chromosome)
    ...
    def compute_children(parents_for_breeding):
        ...
        for parent_1, parent_2 in zip(parents_for_breeding[0::2], parents_for_breeding[1::2]):
            parent_1_bits = parent_1[0:256]
            parent_2_bits = parent_2[256:]

            child = parent_1_bits + parent_2_bits
            children.append(child)

        return children

    children = compute_children(parents_for_breeding)
    breeded_solutions = parents_for_breeding + children

    def match_with_feature_maps(breeded_solutions):
        new_features_solutions = list()
        new_reference_solutions = list()

        for solution in breeded_solutions:
            new_f_solution = list()
            new_r_solution = list()
            ...
            for idx, bit in enumerate(solution):
                if bit == "1":
                    ...
                    ref_bit = total_content_reference_features[:,:, idx]
                    new_f_solution.append(feat_bit)
                    new_r_solution.append(ref_bit)

            new_features_solutions.append(tf.stack(new_f_solution, axis=2))
            new_reference_solutions.append(tf.stack(new_r_solution, axis=2))

        return(new_features_solutions, new_reference_solutions)

    new_generation_pool = match_with_feature_maps(breeded_solutions)
...
for layer_name in Learner.feature_layers:
    ...
    if dims[1] == 50:
        for i in xrange(0, 20):     # number of generations
            ...
            total_solutions = original_pool[2]
            ...
            for style_reference_features, style_combination_features, solution in zip(total_style_reference_features, total_style_combination_features, total_solutions):
                ...
                sol_dictionary[str(solution)] = final_loss


            filter_and_make_new_generation(original_total_style_reference_features, original_total_style_combination_features, sol_dictionary)

            #tmp = filter_and_make_new_generation(feat_dictionary, ref_dictionary, original_total_style_reference_features, original_total_style_combination_features, total_style_reference_features, total_style_combination_features)
