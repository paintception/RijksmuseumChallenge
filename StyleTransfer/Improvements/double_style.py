...
def get_base_image():
    return K.variable(load_image("mat.jpg"))

def get_style_image_1():
    return K.variable(load_image("munch2.jpg"))

def get_style_image_2():
    return K.variable(load_image("picasso.jpg"))
...
def combine_into_tensor(base_image, style_reference_image_1, style_reference_image_2, combination_image):
    return K.concatenate([base_image, style_reference_image_1, style_reference_image_2, combination_image], axis=0)
...
def run_experiment(x):
    ...
    for i in range(100):
        ...
        fname = result_prefix + 'sec_at_iteration_%d.png' % i
...
style_reference_image_1 = get_style_image_1()
style_reference_image_2 = get_style_image_2()

combination_image = K.placeholder((1, img_nrows, img_ncols, 3))

input_tensor = combine_into_tensor(base_image, style_reference_image_1, style_reference_image_2, combination_image)
model = load_model()
...
feature_layers_1 = ['block1_conv1', 'block2_conv1', 'block3_conv1']
feature_layers_2 = ['block1_conv1', 'block2_conv1', 'block3_conv1', 'block4_conv1', 'block5_conv1']

w_sl1 = 0.50
w_sl2 = 0.50

for layer_name_1, layer_name_2 in zip(feature_layers_1, feature_layers_2):
    
    layer_features_1 = outputs_dict[layer_name_1]
    layer_features_2 = outputs_dict[layer_name_2]

    style_reference_features_1 = layer_features_1[1, :, :, :]
    combination_features_1 = layer_features_1[3, :, :, :]
    
    sl_1 = prepare_style_loss(style_reference_features_1, combination_features_1)
    
    style_reference_features_2 = layer_features_2[2, :, :, :]
    combination_features_2 = layer_features_2[3, :, :, :]

    sl_2 = prepare_style_loss(style_reference_features_2, combination_features_2)

    loss += ((w_sl1 / len(feature_layers_1)) * sl_1) + ((w_sl2 / len(feature_layers_2)) * sl_2)
